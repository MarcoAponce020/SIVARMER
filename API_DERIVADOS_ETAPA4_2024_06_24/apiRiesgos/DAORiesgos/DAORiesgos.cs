        ...
        public async Task<Respuesta> FlujosPosicionesPrimariasProcess(int fechaConsulta, List<FlujosPosicionesPrimarias> apiDataList, OracleConnection conexion)
        {
            ...
            try
            {
                ...
                foreach (var groupCOperacion in mirrorDataGroups)
                {
                    ...
                    {
                        respuesta = this.R15_SaveFlujosPosicionesPrimarias(groupCOperacion.ToList(), groupCOperacion.ToList(), fechaConsulta.ToString(), conexion);
                    ...
                    if (currentApiDataList_B.Count != currentProductionList_B.Count)
                    {
                        ...
                        respuesta =  this.R15_SaveFlujosPosicionesPrimarias(currentApiDataList_B, currentApiDataList_B, fechaConsulta.ToString(), conexion);
                    ...
                    else
                    {
                        ...
                        {
                            respuesta = this.R15_SaveFlujosPosicionesPrimarias(newDataList, currentApiDataList_B, fechaConsulta.ToString(), conexion);
                            response.ListaMensajes.Add(respuesta.mensaje);
                        }
                        else {
                            respuesta = this.R15_SaveFlujosPosicionesPrimarias(new List<FlujosPosicionesPrimarias>(), currentApiDataList_B, fechaConsulta.ToString(), conexion);
                            response.ListaMensajes.Add(respuesta.mensaje);
                        }
                    }
                    ...
                    {
                        //Si hay diferencia, insertar todos los registros espejo en tabla productiva
                        respuesta = this.R15_SaveFlujosPosicionesPrimarias(currentApiDataList_C, currentApiDataList_C, fechaConsulta.ToString(), conexion);
                        response.ListaMensajes.Add(respuesta.mensaje);
                    }
                    else
                    {
                        ...
                        if (newDataList.Any() && newDataList.Count > 0)
                        {
                            respuesta = this.R15_SaveFlujosPosicionesPrimarias(newDataList, currentApiDataList_C, fechaConsulta.ToString(), conexion);
                            response.ListaMensajes.Add(respuesta.mensaje);
                        }
                        else
                        {
                            respuesta = this.R15_SaveFlujosPosicionesPrimarias(new List<FlujosPosicionesPrimarias>(), currentApiDataList_C, fechaConsulta.ToString(), conexion);
                            response.ListaMensajes.Add(respuesta.mensaje);
                        }
                    }
                }
        ...
        private async Task<List<PosicionPrimariaSwaps>> R18_GetProductionData(int position, string fechaConsulta, OracleConnection conexion)
        {
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        cOperacion = reader["COperacion"].ToString()!;
                        cPosicion = reader["CPosicion"].ToString()!;
