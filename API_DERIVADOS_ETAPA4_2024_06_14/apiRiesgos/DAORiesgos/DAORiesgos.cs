...
namespace DAO
{
    public class DAORiesgos
    {
        ...
        private List<OracleParameter> R15_GetParameters(FlujosPosicionesPrimarias item)
        {
            ...
            parameterList.Add(new OracleParameter("HORAREG", "00000")); //DateTime.UtcNow.ToString("HH:mm:ss")
        ...
        private Respuesta R15_SaveFlujosPosicionesPrimarias(List<FlujosPosicionesPrimarias> dataList,string fechaConsulta, OracleConnection conexion)
        {
            ...
            try
            {
                ...
                foreach (FlujosPosicionesPrimarias item in dataList)
                {
                    ...
                    cmd.BindByName = true;
                    cmd.CommandText = this.R15_GetStringSQLProduction("RIESVARM.VAR_TD_FLUJOS_SWAPS", item.FechaLiq, item.F_Desc);
            ...
            catch (Exception ex)
            {
                ...
                //response.mensaje = $"{recordsInsertedProduction} registros fueron revertidos. " + "Hubo un error al insertar los registros. " + ex.Message;
