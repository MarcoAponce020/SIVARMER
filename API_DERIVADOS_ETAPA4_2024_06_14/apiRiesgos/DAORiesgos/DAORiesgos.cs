...
namespace DAO
{
    public class DAORiesgos
    {
        ...
        public Respuesta guardarFlujosSwaps(List<FlujosSwaps> reporte, List<CatalogoDivisas> catalogoDivisas, OracleConnection conexion)
        {
            ...
            bool borro = this.R14_EliminarFlujoswaps(Convert.ToInt32(reporte[0].Fecha_Pos), conexion);
            if (!borro)
            {
                respuesta.mensaje = "Hubo un error al borrar los registros.";
                return respuesta;
        ...
        public bool R14_EliminarFlujoswaps(int fechaConsulta, OracleConnection conexion)
        {
            ...
            try
            {
                ...
                cmd.CommandText = String.Format("DELETE FROM XYZWIN.SW_RIESGOS1");
                cmd.ExecuteNonQuery();
        ...
        private async Task<List<FlujosPosicionesPrimarias>> R15_GetProductionData(OracleConnection conexion)
        {
            ...
            try
            {
                string strSQL = "SELECT * FROM RIESVARM.VAR_TD_FLUJOS_SWAPS WHERE TIPOPOS = 1 AND CPOSICION = 50 AND T_PATA IN('B', 'C')";
                StringBuilder sbSQL = new StringBuilder();
                sbSQL.AppendLine("SELECT * FROM RIESVARM.VAR_TD_FLUJOS_SWAPS ");
                sbSQL.AppendLine("WHERE TIPOPOS = 1 ");
                sbSQL.AppendLine("AND CPOSICION = 50 ");
                sbSQL.AppendLine("AND T_PATA IN('B', 'C') ");
                sbSQL.AppendLine("AND FECHAREG = (SELECT MAX(FECHAREG) FROM RIESVARM.VAR_TD_FLUJOS_SWAPS ");
                sbSQL.AppendLine("                WHERE TIPOPOS = 1 ");
                sbSQL.AppendLine("                AND CPOSICION = 50 ");
                sbSQL.AppendLine("                AND T_PATA IN('B', 'C'))");
