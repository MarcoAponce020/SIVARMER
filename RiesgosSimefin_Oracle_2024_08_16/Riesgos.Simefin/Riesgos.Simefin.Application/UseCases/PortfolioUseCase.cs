...
namespace Riesgos.Simefin.Application.UseCases
{
    ...
    public class PortfolioUseCase : IPortfolioUseCase
    {
        ...
        private enum OriginEnum { Endpoint, Server };
        ...
        public async Task<ResponseDTO> ExcelLoad(IFormFile file)
        {
            ...
            try
            {
                ...
                if (this.procedencia == OriginEnum.Server.ToString()) //Servidor
                {
        ...
        private ResponseDTO ValidateFileExist(ref IFormFile file)
        {
            ...
            this.procedencia = OriginEnum.Endpoint.ToString(); //Endpoint
            string pathOrigin = Path.Combine(this.uploadServerFileUrlPath, this.uploadServerFolderSource);

            try
            {
                //Si "file" viene en el Endpoint, validar como exitoso
                ...
                //Si "file" no viene en el Endpoint, buscar en carpeta de servidor
                if (file == null || file.Length == 0)
                {
                    string serverPathWithFile = Path.Combine(pathOrigin, this.fileName);

                    //Determina si existe el archivo
                    bool fileExist = this.GetFileExist();
                    if (fileExist)
                    {
                        //Si el archivo es un CSV
                        if (serverPathWithFile.ToLower().EndsWith(".csv"))
                        {
                            ...
                            {
                                response.Mensaje = "El contenido o formato (content-type) del archivo no es el esperado. Método: ValidateFileExist.";
                                response.StatusCode = HttpStatusCode.BadRequest;
                                return response;
                            }

                            //Obteniendo archivo de servidor
                            string name = Path.GetFileNameWithoutExtension(serverPathWithFile);
                            string fileContent = File.ReadAllText(serverPathWithFile);
                            ...
                            file = archivoForm;
                            response.IsExitoso = true;
                            response.Mensaje = $"El archivo {serverPathWithFile} fué encontrado. Método: ValidateFileExist.";
                            response.StatusCode = HttpStatusCode.OK;
                            this.procedencia = OriginEnum.Server.ToString(); //Servidor
                        }
                        else
                        {
                            response.Mensaje = $"El archivo {serverPathWithFile} no es un .csv. Método: ValidateFileExist.";
                            response.StatusCode = HttpStatusCode.BadRequest;
                            return response;
                        }
                    }
                    ...
                    {
                        response.Mensaje = $"El archivo {serverPathWithFile} no existe. Método: ValidateFileExist.";
                        response.StatusCode = HttpStatusCode.BadRequest;
                        return response;
                    }
                }
                ...
                if (file == null || file.Length == 0)
                {
                    response.Mensaje = "No se proporcionó un archivo. Método: ValidateFileExist.";
                    response.StatusCode = HttpStatusCode.BadRequest;
                }
            }
            ...
            {
                response.IsExitoso = false;
                response.Mensaje = $"La ruta {pathOrigin} no existe. Método: ValidateFileExist.";
                response.StatusCode = HttpStatusCode.BadRequest;
            }
            catch (IOException)
            {
                response.IsExitoso = false;
                response.Mensaje = $"El archivo en la ruta {pathOrigin} se encuentra bloqueado por otro programa. Método: ValidateFileExist.";
                response.StatusCode = HttpStatusCode.Locked;
            }
            catch (UnauthorizedAccessException)
            {
                response.IsExitoso = false;
                response.Mensaje = $"No cuenta con permisos de acceso a la ruta {pathOrigin}. Método: ValidateFileExist.";
                response.StatusCode = HttpStatusCode.Unauthorized;
            }
            catch (Exception ex)
            {
                response.IsExitoso = false;
                response.Mensaje = $"{ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: ValidateFileExist.";
                response.StatusCode = HttpStatusCode.NotFound;
            }
            return response;
        }
        ...
        private async Task<ResponseDTO> MoveFileToServerFolder()
        {
            ...
            string destinationFileName = $"{name}-[{date}]{extension}";

            string pathOrigin = Path.Combine(this.uploadServerFileUrlPath, this.uploadServerFolderSource);
            string pathDestination = Path.Combine(this.downloadServerFileUrlPath, this.downloadServerFolderDestination);

            string sourceFile = Path.Combine(pathOrigin, this.fileName);
            string destinationFile = Path.Combine(pathDestination, destinationFileName);
            bool fileExist = false;
            
            try
            {
                //Determina si existe el archivo
                fileExist = this.GetFileExist();
                if (fileExist)
                {
                ...
                {
                    response.Mensaje = $"El archivo {sourceFile} no existe. Método: MoveFileToServerFolder.";
                    response.StatusCode = HttpStatusCode.BadRequest;
                }
            }
            ...
            {
                response.IsExitoso = false;
                response.Mensaje = $"La ruta {pathOrigin} o {pathDestination} no existe. {ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: MoveFileToServerFolder.";
                response.StatusCode = HttpStatusCode.BadRequest;
            }
            catch (IOException ex)
            {
                response.IsExitoso = false;
                response.Mensaje = $"El archivo {sourceFile} se encuentra bloqueado por otro programa. {ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: MoveFileToServerFolder.";
                response.StatusCode = HttpStatusCode.Locked;
            }
            catch (UnauthorizedAccessException ex)
            {
                response.IsExitoso = false;
                response.Mensaje = $"No cuenta con permisos de acceso a la ruta {pathOrigin} o {pathDestination}. {ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: MoveFileToServerFolder.";
                response.StatusCode = HttpStatusCode.Unauthorized;
            }
            catch (Exception ex)
            {
                response.Mensaje = $"{ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: MoveFileToServerFolder.";
                response.StatusCode = HttpStatusCode.BadRequest;
            }
        ...
        private async Task<ResponseDTO> SaveBackUpServer()
        {
            ...
            try
            {
                ...
                if (fileExist)
                {
                    byte[] bytes = await File.ReadAllBytesAsync(serverPathWithFile);
                ...
                {
                    response.Mensaje = "El archivo " + serverPathWithFile + " no existe. Método: SaveBackUpServer.";
                    response.StatusCode = HttpStatusCode.BadRequest;
                }
            }
            catch (DirectoryNotFoundException)
            {
                response.IsExitoso = false;
                response.Mensaje = $"La ruta {pathOrigin} no existe. Método: SaveBackUpServer.";
                response.StatusCode = HttpStatusCode.BadRequest;
            }
            catch (IOException)
            {
                response.IsExitoso = false;
                response.Mensaje = $"El archivo {serverPathWithFile} se encuentra bloqueado por otro programa. Método: SaveBackUpServer.";
                response.StatusCode = HttpStatusCode.Locked;
            }
            catch (UnauthorizedAccessException)
            {
                response.IsExitoso = false;
                response.Mensaje = $"No cuenta con permisos de acceso a la ruta {pathOrigin}. Método: SaveBackUpServer.";
                response.StatusCode = HttpStatusCode.Unauthorized;
            }
            catch (Exception ex)
            {
                response.Mensaje = $"{ExceptionMessageHelper.GetExceptionMessage(ex)}. Método: SaveBackUpServer.";
                response.StatusCode = HttpStatusCode.BadRequest;
            }
        ...
        /// <summary>
        /// Buscar en ruta del servidor la existencia de archivo
        /// </summary>
        /// <returns></returns>
        private bool GetFileExist()
        {
            ...
            string serverPath = Path.Combine(this.uploadServerFileUrlPath, this.uploadServerFolderSource);
            string serverPathWithFile = Path.Combine(this.uploadServerFileUrlPath, this.uploadServerFolderSource, this.fileName);
            var files = Directory.GetFiles(serverPath, "*.csv", new EnumerationOptions { IgnoreInaccessible = true, RecurseSubdirectories = false });
            fileExist = files.Contains(serverPathWithFile);
