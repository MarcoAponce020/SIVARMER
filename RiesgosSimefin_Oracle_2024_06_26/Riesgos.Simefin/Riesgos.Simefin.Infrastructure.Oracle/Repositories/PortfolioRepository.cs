...
namespace Riesgos.Simefin.Infrastructure.Oracle.Repositories
{
    ...
    public class PortfolioRepository : IPortfolioRepository
    {
        ...
        public async Task<bool> SaveFileBase64(string originalName, string compoundName, byte[] stringBase64, string origen = "SERVER")
        {
            ...
            //var sql = "INSERT INTO MiTabla (MiColumnaBLOB) VALUES (:blobData)";
            //using (var command = new OracleCommand(sql, connection))
            //{
            //    command.Parameters.Add(":blobData", OracleDbType.Blob).Value = imageBytes;
        ...
        public async Task<byte[]> ReadOracleBLOB(string fileDate)
        {
            byte[] fileBytes = [];

            //OracleCommand cmd = new OracleCommand(@$"SELECT Archivo_Base64 FROM {_historialTableName} WHERE Fecha = :pFecha", _connectionString);
            //cmd.Parameters.Add("pFecha", OracleDbType.Int32).Value = fecha;

            //OracleDataReader reader = cmd.ExecuteReader();
            //reader.Read();

            //OracleBlob blob = reader.GetOracleBlob(0); // Obtén el BLOB
            //var streamReader = new System.IO.StreamReader(blob);
            //string content = streamReader.ReadToEnd();

            using (var connection = new OracleConnection(_connectionString))
            {
                //await connection.OpenAsync();
                //var parameters = new { pFecha = fileDate };
                string strQuery = @$"SELECT Archivo_Base64
                                    FROM {_historialTableName} 
                                    WHERE to_date(Fecha,'dd/mm/yy') = to_date(:pFecha,'dd/mm/yy')"; //to_date(:pFecha,'yyyymmdd')

                //////OracleCommand oracleCommand = new OracleCommand();
                //////oracleCommand.CommandText = strQuery;
                //////oracleCommand.CommandType = CommandType.Text;

                //////OracleDataReader reader = await connection.ExecuteReaderAsync(oracleCommand, CommandBehavior.Default);
                ////////cmd.ExecuteReader();
                //////reader.Read();
                ...
                using (var cmd = new OracleCommand())
                {
                    ...
                    cmd.Parameters.Add("pFecha", fileDate);
