...
namespace Riesgos.Simefin.Application.UseCases
{
    ...
    public class PortfolioUseCase : IPortfolioUseCase
    {
        ...
        private string[] originList = [ "endpoint", "server" ];
                        ...
                        for (int col = 1; col <= colCount; col++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(currentCellValue))
                            {
                                ...
                                if (col == 1)
                                {
                                    ...
                                    byte[] bytes = Encoding.Default.GetBytes(subPortfolio);
        ...
        private ResponseDTO ValidateFileExist(ref IFormFile file)
        {
            ...
            if (file == null || file.Length == 0)
            {
                ...
                //string urlLocal = @"D:\Banobras\Sistemas .NET\SST3\SIVARMER\P&L_2024_08_31.csv";
                var serverFileUrlPath = _configuration.GetSection("ServerFileUrlPath").Value;
                var nameSourceFolderServer = _configuration.GetSection("NameSourceFolderServer").Value;
                var fileName = _configuration.GetSection("FileName").Value;
                var contentType = _configuration.GetSection("ContentTypeFile").Value;
                string pathServerFolderComplete = @$"{serverFileUrlPath}{nameSourceFolderServer}{fileName}";

                //Determina si existe el archivo
                if (System.IO.File.Exists(pathServerFolderComplete))
                {
        ...
        private async Task<ResponseDTO> MoveFileToServerFolder()
        {
            ...
            var serverFileUrlPath = _configuration.GetSection("ServerFileUrlPath").Value;
            var nameSourceFolderServer = _configuration.GetSection("NameSourceFolderServer").Value;
            var nameDestinationFolderServer = _configuration.GetSection("NameDestinationFolderServer").Value;
            var fileName = _configuration.GetSection("FileName").Value;
            ...
            string sourcePath = @$"{serverFileUrlPath}{nameSourceFolderServer}{fileName}";
            string destinationPath = @$"{serverFileUrlPath}{nameDestinationFolderServer}{name}-[{date}]{extension}";
        ...
        private async Task<ResponseDTO> SaveBackUpServer()
        {
            ...
            try
            {
                ...
                if (System.IO.File.Exists(sourcePath))
                {
                    ...
                    //String base64String2 = Convert.ToBase64String(bytes);
                    ...
                    var result = await _portfolioRepository.SaveFileBase64(originalName, compoundName, imageBytes, "SERVER");
        ...
        /// </summary>
        /// <param name="dateFile">Fecha de almacenamiento</param>
        /// <returns></returns>
        public async Task<byte[]> GetFileCSV(string fileDate)
        {
            var fileBytes = await _portfolioRepository.ReadOracleBLOB(fileDate);
            ...
            //return await Task.FromResult(File(fileBytes, "text/csv", fileName));
