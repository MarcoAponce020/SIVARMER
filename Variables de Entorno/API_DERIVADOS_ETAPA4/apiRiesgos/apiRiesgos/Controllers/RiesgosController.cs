...
namespace apiRiesgos.Controllers
{
    ...
    public class RiesgosController : ControllerBase
    {
        ...
        private readonly OracleConnection? connDerivados;

        public RiesgosController(IServicioApi servicioApi, ILogger<RiesgosController> logger, IConfiguration _configuration)
        {
            ...
            try
            {
                ...
                conn = getOracleConnection.GetConnection(EnumDataBaseTypes.VARMER);
                connDerivados = getOracleConnection.GetConnection(EnumDataBaseTypes.DERIVADOS);
            }
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    switch (tipoReporte)
                    {
                        ...
                        case 17:
                            ...
                            {
                                respuesta = new DALRiesgos().guardarLlamadaMargen(respuestaAPI.ListaLlamadaMargen, conn!);
                            }
                            break;
                        case 18:
