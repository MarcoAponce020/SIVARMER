...
namespace apiRiesgos.Servicios
{
    public class ServicioApi : IServicioApi
    {
        ...
        private readonly string? _urlCom;
        private readonly string? _urlTok;
        private readonly string? _urlRiesgos;
        ...
        {
            ResultadoCredencial resultado = new ResultadoCredencial { exito = false };
            ...
            try
            {
                ...
                string body = $"grant_type={_grant_type}&client_id={client_id}&client_secret={client_secret}&scope={_scope}";
        ...
        public async Task<ResultadoApi> GetRiesgos(int tipoReporte, int fechaConsulta, string token)
        {
            ...
            try
            {
                ...
                string urlTmp = $"{client.BaseAddress?.ToString()}/{tipoReporte.ToString()}/{fechaConsulta.ToString()}";
                Uri myUri = new Uri(urlTmp);
        ...
        private HttpClient Method_Headers(string accessToken, string endpointURL)
        {
            ...
            {
                UseDefaultCredentials = false, //Instrucción que nos ayuda a omitir el certificado seguro
        ...
        public async Task<ResultadoApi> GetRiesgosRango(int tipoReporte, int fechaIni, int fechaFin, string token)
        {
            string serviceUrl = _urlRiesgos + "/api/riesgos";
            ...
            try
            {
                ...
                ////HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, Uri.EscapeDataString(client.BaseAddress.ToString()));
                ////request.Content.Headers.ContentType = new MediaTypeHeaderValue("application/json");

                string urlTemp = $"{client.BaseAddress?.ToString()}/{tipoReporte.ToString()}/{fechaIni.ToString()}/{fechaFin.ToString()}";
                HttpResponseMessage tokenResponse = await client.GetAsync(Uri.EscapeDataString(urlTemp));
