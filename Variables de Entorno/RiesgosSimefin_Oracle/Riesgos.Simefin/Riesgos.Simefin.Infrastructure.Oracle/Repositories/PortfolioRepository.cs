...
namespace Riesgos.Simefin.Infrastructure.Oracle.Repositories
{
    ...
    public class PortfolioRepository : IPortfolioRepository
    {
        ...
        private readonly string _historialTableName = string.Empty;
        ...
        public PortfolioRepository(IConfiguration configuration)
        {
            ...
            _historialTableName = _configuration.GetSection("DataBaseTables").GetSection("T_Historial").Value!;
        }
        ...
        {
            using (IDbConnection connection = OracleConnectionHelper.GetConnection())
            {
                ...
                string strOrder = " ORDER BY A.IdPortafolio ASC";
                var portfolios = await connection.QueryAsync<Portafolio>(strQuery + strOrder, filterList);
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            foreach (var item in portafoliosList)
                            {
                                ...
                                {
                                    var currentPortolio = portafolios.FirstOrDefault();
                                    currentPortolio!.ListaDatos = item.ListaDatos;
                                    await this.UpdateAsync(currentPortolio);
                                }
                                else
                                {
        ...
        /// <returns></returns>
        public async Task<bool> SaveFileBase64(string originalName, string compoundName, byte[] stringBase64, string origen = "SERVER")
        {
            using (var connection = OracleConnectionHelper.GetConnection())
            {
                await connection.OpenAsync();
                var parameters = new
                {
                    p_Fecha = DateTime.UtcNow,
                    p_NombreOrigen = originalName,
                    p_NombreDestino = compoundName,
                    p_Archivo = stringBase64,
                    p_Origen = "SERVER"
                };
                var query = @$"INSERT INTO {_historialTableName} (Fecha, Nombre_Origen, Nombre_Destino, Archivo_Base64, Origen) 
                            VALUES(:p_Fecha, :p_NombreOrigen, :p_NombreDestino, :p_Archivo, :p_Origen)";
                var affectedRows = await connection.ExecuteAsync(query, parameters);
                return affectedRows > 0;
            }        
        }
        ...
        /// <returns></returns>
        public async Task<ResponseDTO> ReadOracleBLOB(string fileDate)
        {
            ResponseDTO response = new ResponseDTO { IsExitoso = false };

            string folderDownload = @$"{_configuration.GetSection("FileDownload").GetSection("NameFolderDownload").Value!}\";
            string userRoot = Environment.GetEnvironmentVariable("USERPROFILE")! + @"\";
            var date = DateTime.Now.ToString("s").Replace(":", ".");
            string fileName = $"{userRoot}{folderDownload}portfolio_download_{date}.csv";

            using (OracleConnection connection = OracleConnectionHelper.GetConnection())
            {
                //connection.Open(); 
                await connection.OpenAsync();
                string strQuery = @$"SELECT Archivo_Base64
                                FROM {_historialTableName} 
                                WHERE to_date(Fecha,'dd/mm/yy') = to_date(:pFecha,'dd/mm/yy')";

                using (OracleCommand cmd = new OracleCommand(strQuery, connection))
                {
                    cmd.Parameters.Add(":pFecha", OracleDbType.Varchar2).Value = fileDate;

                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            // Ensure the column index matches the position of BLOB_COLUMN in your SELECT query
                            OracleBlob blob = reader.GetOracleBlob(0); // 0 is the index of the BLOB_COLUMN in the query result

                            if (!reader.IsDBNull(0))
                            {
                                using (FileStream fs = new FileStream(fileName, FileMode.Create))
                                {
                                    fs.Write(blob.Value, 0, (int)blob.Length);
                                    response.IsExitoso = true;
                                    response.Mensaje = $"El archivo puede ser ubicado en {fileName}";
                                    response.StatusCode = HttpStatusCode.OK;
                                }
                            }
                            else
                            {
                                //Console.WriteLine("BLOB column is NULL.");
                                response.IsExitoso = false;
                                response.Mensaje = $"El archivo de fecha {fileDate} no tiene contenido.";
                        ...
                        {
                            response.IsExitoso = false;
                            response.Mensaje = $"No fue posible recuperar el archivo con fecha {fileDate}";
