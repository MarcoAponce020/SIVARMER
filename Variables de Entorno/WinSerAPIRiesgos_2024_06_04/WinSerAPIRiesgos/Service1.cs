...
namespace WinSerAPIRiesgos
{
    public partial class Service1 : ServiceBase
    {
        ...
        public async Task<string> GetToken()
        {
            ...
            {
                token = $"Error al generar TOKEN. {ex.Message}";
                log.Error(token);
            }
        ...
        public async Task<string> ejecutarReporte(int tipo,string nombre, string fecha, string token)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    //GET Method
                    HttpResponseMessage response = await client.GetAsync($"{ tipo }/{ fecha }/{ token }");
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async Task EnviarEvidencia(string comentarios, string fecOper)
        {
            ...
            {
                response.mensaje = $"Hubo un error al enviar el correo. {ex.Message}";
                log.Error(ex);
